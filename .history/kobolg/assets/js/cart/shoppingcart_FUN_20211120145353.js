...
Printersitems.addEventListener("click", function (e) {
  const filteredItems = products.filter(item => item.type === e.currentTarget.dataset.type);
...
const compactfor (let index = 0; index < products.length; index++) {
...
}tems = document.getElementById("compact");

compactfor (let index = 0; index < products.length; index++) {
  document.getElementById(products[index].id).onclick = () => {
    var name = products[index].name;
    var price = products[index].price;
    var quentity = products[index].quantity;
...
}tems.addEventListener("click", function (e) {
  const filteredItems = products.filter(item => item.type === e.currentTarget.dataset.type);
