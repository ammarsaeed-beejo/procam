...
maproductulin.innerHTML = localStorageData.reduce((acc, products) => {
  acc += `
    ...
    document.getElementById(products.id).addEventListener("click" , () =>{
          const adam = localStorage.setItem("item-name", products.name);
          const adam2 = localStorage.setItem("item-price", products.price);
          const adam3 = localStorage.setItem("item-img", products.img);
          const adam4 = localStorage.setItem("item-info", products.info);
    })
